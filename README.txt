...
------------------------------------------------------------
13. PERBAIKAN PDF AGAR TIDAK AUTO-DOWNLOAD
------------------------------------------------------------

Versi ini menggunakan PDF.js untuk merender PDF menjadi canvas.
PDF tidak lagi dimasukkan langsung menggunakan iframe/raw URL.

Artinya:

    PDF GitHub
       ↓
    PDF.js mengambil isi file
       ↓
    PDF dirender menjadi canvas
       ↓
    Canvas tampil sebagai preview

Browser tidak diarahkan ke URL PDF, sehingga tidak seharusnya muncul
download otomatis ketika halaman dibuka atau ketika sertifikat diklik.

Jika browser masih menyimpan file PDF otomatis, pastikan kamu sudah
menggunakan `index.html`, `script.js`, dan `style.css` dari versi revisi
ini, bukan file versi sebelumnya.
